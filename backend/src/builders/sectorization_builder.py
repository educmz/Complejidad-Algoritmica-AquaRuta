...
def _ordered_groups(grouped_zones: list[dict]) -> list[dict]:
    return sorted(grouped_zones, key=lambda group: group.get("prioridad_score", 0), reverse=True)


def build_sectorized_zones(districts_summary: list[dict], grouped_zones: list[dict]) -> dict:
    zone_lookup = _build_zone_lookup(districts_summary)
    result = {}
    for group in _ordered_groups(grouped_zones):
        group_zones = [
            zone_lookup[zone_id]
            for zone_id in group.get("zona_ids", [])
        ...
        criterios = {"geografico": {}, "carga": {}, "mixto": {}}
        for count in sector_options:
            for criterion in criterios:
                criterios[criterion][str(count)] = sectorizar_divide_conquer(
                    group_zones,
                    sector_count=count,
                    criterion=criterion,
                )
        result[group["id"]] = {
            ...
            "personas_afectadas_estimadas": group.get("personas_afectadas_estimadas", 0),
            "peso_demanda_familiar": group.get("peso_demanda_familiar", 0),
            "prioridad_score": group.get("prioridad_score", 0),
            "promedio_integrantes_hogar": group.get("promedio_integrantes_hogar", 0),
            "criterios": criterios,
        }
    return result
