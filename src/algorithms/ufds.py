...
def _has_connectivity(metrics, config):
    ...
    reasonable_route = time_ok and cost_ok
    connected = road_ok or reasonable_route
...
def _candidate_search_radius(config):
    # For candidate discovery, use the broadest threshold. Connectivity is
    # checked later before union.
...
def _evaluate_connection(zone_a, zone_b, config):
    ...
    if not close and not connected:
        reason = f"{close_reason}; {connectivity_reason}"
    elif not close:
        reason = close_reason
    else:
...
def should_connect(zone_a, zone_b, **kwargs):
    """
    Compatibilidad publica: retorna True solo si el par cumple cercania y
    conectividad operativa. No usa departamento/provincia como regla de union.
    """
    config = _merge_config(**kwargs)
    return _evaluate_connection(zone_a, zone_b, config)["should_union"]
...
def _node_summary(zone, union_count, candidate_count, rejected):
    ...
    if not has_unions and not rejected:
        if candidate_count == 0:
            reason = "no se encontraron vecinos cercanos dentro de los umbrales configurados"
        else:
            reason = "no se encontro union valida con los vecinos candidatos evaluados"
        rejected = [
            {
...
def build_groups_from_ufds(zones, ufds: UFDS, explainability, config):
    ...
    for index in range(len(zones)):
        root = ufds.find(index)
        grouped_indices.setdefault(root, []).append(index)

    groups = []
    union_edges = explainability["union_edges"]
    per_node_union_count = explainability["per_node_union_count"]
    per_node_candidate_count = explainability["per_node_candidate_count"]
    per_node_rejections = explainability["per_node_rejections"]
    ...
    for group_number, indices in enumerate(grouped_indices.values(), start=1):
        ...
        group_edges = [
            ...
            if edge["source"] in zone_ids and edge["target"] in zone_ids
        ]
        nodes = [
            _node_summary(
                zone,
                per_node_union_count.get(_zone_value(zone, "id"), 0),
                per_node_candidate_count.get(_zone_value(zone, "id"), 0),
                per_node_rejections.get(_zone_value(zone, "id"), []),
            )
            for zone in group_zones
        ]
    ...
    groups = sorted(groups, key=lambda group: group["interrupciones"], reverse=True)
...
def agrupar_zonas_operativas(
    ...
    """
    Agrupa zonas con UFDS usando cercania + conectividad.

    Parametros legados como same_department/same_province se aceptan para no
    romper llamadas existentes, pero no participan en la regla de union.
    """
    ...
    for source_index, target_index in candidate_pairs:
        ...
        if result["should_union"]:
            did_union = ufds.union(source_index, target_index)
            if did_union:
                edge = {
                    "source": _zone_value(source, "id"),
                    "sourceName": _zone_name(source),
    ...
    # Attach compact global context to every group so the frontend can explain
    # the final partition without changing grouped_zones.json from array to obj.
    isolated_groups = sum(1 for group in groups if group["es_aislado"])
    small_non_isolated_groups = sum(
        1
        for group in groups
        if not group["es_aislado"] and int(group.get("cantidad_nodos", 0) or 0) <= 2
    )
    ...
    global_summary = {
        ...
        "grupos_pequenos_no_aislados": small_non_isolated_groups,
