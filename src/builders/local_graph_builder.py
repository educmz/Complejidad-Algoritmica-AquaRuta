...
def _build_sector_edges(nodes, neighbors_per_node=3):
    ...
    for source in nodes:
        ...
        for target in nodes:
            ...
            candidates.append({
                "target": target,
                "weight": _distance(source.get("center"), target.get("center")),
            })

        candidates = [
            item for item in candidates if item["weight"] != float("inf")
        ]
    ...
    for group_id, group in sectorized_zones.items():
        ...
        for sector in sectors:
            ...
            group_result["sectors"][sector_key] = {
                ...
                "routes": {
                    criterion: _solve_sector_tsp(origin, nodes, criterion)
